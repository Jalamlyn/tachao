...
import { getCurrentPosition, getAddressFromLocation } from "@/lib/locationUtils"
import { getLocationPermissionGuide } from "@/lib/browserUtils"
...
import { toMarkdown } from "@/lib/chat-to-markdown"
import { searchExa, getExaContents } from "@/lib/chat-exa"
...
const wpm = {
  ...
  import: async (moduleName: string) => {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => {
        const error = new Error(`模块 ${moduleName} 或者内部依赖模块未实现，请检查`)
