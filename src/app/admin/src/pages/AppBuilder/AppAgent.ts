...
请先分析用户输入和各个模块的关系，将分析结果输出到 mo-ai-think 标签中，然后将相关模块ID以JSON格式返回到 <mo-ai-code> 标签中。JSON格式为：{"moduleIds": ["id1", "id2"]}。只返回确实相关的模块ID。`
  ...
  public async processCommand(
    ...
    try {
      // 检查是否是 PM 对话
      const commandContent = typeof command === "string" ? command : command.content
      if (commandContent.trim().toLowerCase().startsWith("@pm")) {
        // 使用 ProductManagerAgent 处理
        return ProductManagerAgent.processCommand(
          appId,
          messages,
          typeof command === "string" ? command : command.content,
          onChunk
        )
      }
...
<我的输入>${typeof command === "string" ? command : command.content},生成的代码要完整, 不要省略</我的输入>,4. 
  仔细分析我的输入,考虑以下方面:
  - 任务复杂性
  - 上下文信息审查
  - 执行计划制定
