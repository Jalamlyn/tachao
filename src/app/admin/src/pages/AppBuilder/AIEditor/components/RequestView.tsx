                ...
                {requestStore.latestRequests.map((request) => (
                  ...
                  >
                    <Card
                      key={request.id}
                      isPressable
                      className={`w-full p-3 cursor-pointer transition-all duration-200 hover:scale-[1.02] ${
                        selectedRequest?.id === request.id ? "bg-primary-50 border-primary" : "hover:bg-default-100"
                      }`}
                      onPress={() => {
                    ...
                    >
                      <div className='flex items-center justify-between'>
                        <div className='flex items-center gap-2'>
                          ...
                          <span className='text-small text-default-500'>{formatTime(request.timestamp)}</span>
                        </div>
                        <Button
                          size='sm'
                          variant='light'
                          ...
                          <Tooltip
                            content={
                              <div className='whitespace-pre-wrap'>
                                {request.method === "GET" ? request.params.names.join(", ") : request.params.name}
                              </div>
                            }
                            placement='bottom'
                          >
                            <div className='text-small text-default-600 max-w-36 break-words'>
                              {request.method === "GET"
                ...
                >
                  <div>
                    <div className='flex items-center justify-between mb-2'>
                      ...
                      <Chip size='sm' variant='flat' color={selectedRequest.method === "GET" ? "primary" : "success"}>
                          ...
                          navigator.clipboard.writeText(
                            formatJson(
                              selectedRequest?.response?.value
                                ? selectedRequest?.response?.value
                                : selectedRequest?.response?.data[0]?.value || `{}`
                            )
                          )
                          // 可以添加复制成功的提示
                        }}
