...
export interface ModuleIndex {
  ...
  type: string // app/page/store/service/module
  name: string // 模块名称
  title?: string // 显示标题
}
...
class AppCodeStore {
  ...
  // 创建新的App
  async createApp(name: string): Promise<any> {
    const appId = this.generateId()
    this.setAppId(appId)

    try {
      const firstVersion = await this.handleAIGeneration(initialAIResponse, name)
