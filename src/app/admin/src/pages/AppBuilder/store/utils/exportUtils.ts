...
export function exportToMarkdown(version: Version): string {
  let markdown = "# 应用代码导出\n\n"
...
export function downloadMarkdown(version: Version, appId: string): void {
  try {
    const markdown = exportToMarkdown(version)
    const blob = new Blob([markdown], { type: "text/markdown" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url
    a.download = `${appId}_export_${new Date().toISOString().slice(0, 10)}.md`
