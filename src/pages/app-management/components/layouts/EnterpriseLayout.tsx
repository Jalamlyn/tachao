...
import Sidebar from "@/apps/we-chat-app-admin/component/sidebar"
import type { SidebarItem } from "@/apps/we-chat-app-admin/component/sidebar"
...
export const EnterpriseLayout: React.FC = () => {
  ...
  const sidebarItems: SidebarItem[] = [
    // 表单分组
    {
      ...
      items: app.indexFields?.templateIds?.map((templateId, index) => ({
        key: `form-${templateId}`,
        title: `表单 ${index + 1}`,
    ...
    // 报表分组
    {
      ...
      title: "报表管理",
      icon: "mdi:chart-box",
      items: app.indexFields?.reportIds?.map((reportId, index) => ({
        key: `report-${reportId}`,
        title: `报表 ${index + 1}`,
        icon: "mdi:chart-line",
