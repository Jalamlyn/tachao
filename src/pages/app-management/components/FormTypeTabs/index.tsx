...
export const FormTypeTabs: React.FC<FormTypeTabsProps> = ({ forms, isLoading }) => {
  ...
  // 使用 useMetadata hook 获取删除方法
  const { remove: removeForm, load: loadForms } = useMetadata("form")
  ...
  const handleDelete = async (ids: string[]) => {
    try {
      // 批量删除
      await Promise.all(ids.map(id => removeForm(id)))
      ...
      message.error("删除失败")
      throw error // 向上传播错误，让 FormTypeTable 处理loading状态
    }
  }
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              <FormTypeTable
                ...
                onDelete={handleDelete}  // 传递删除处理函数
              />
            </div>
          </Tab>
        ))}
