  ...
  if (!app) {
    return (
      <EmptyState
        type="error"
        title="未找到应用"
        description="该应用可能已被删除或您没有访问权限"
      />
    )
  }

  const appTemplates = templates.filter(
    template => app.indexFields?.templateIds?.includes(template.id)
  )
  const appReports = reports.filter(
    report => app.indexFields?.reportIds?.includes(report.id)
  )
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-small text-default-500">
                          {template.description || '点击开始填写表单'}
                        </p>
                      </div>
                      <Button
                        color="primary"
                        variant="flat"
                        onPress={() => navigate(`/form/${template.id}`)}
                      >
                        开始填写
                      </Button>
                    </div>
