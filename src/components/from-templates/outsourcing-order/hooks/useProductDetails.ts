...
export const useProductDetails = (form: UseFormReturn<OutsourcingOrderFormValues>) => {
  const updateTotalPrice = (index: number) => {
    const products = form.getValues("data.productDetails")
    const product = products[index]
    
    const inboundQuantity = Number(product.inboundQuantity) || 0
    const unitPrice = Number(product.unitPrice) || 0
    const totalPrice = Number((inboundQuantity * unitPrice).toFixed(2))
  ...
  const handleQuantityChange = (index: number, value: number, type: "inbound" | "outbound") => {
    if (type === "inbound") {
