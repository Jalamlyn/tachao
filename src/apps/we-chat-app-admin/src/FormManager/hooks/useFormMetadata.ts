...
export const useFormMetadata = () => {
  ...
  const getTemplateDetail = useCallback(async (templateId: string) => {
    try {
      const templateDetailResult = await getMetadata([`template_${templateId}`])
      if (templateDetailResult.data && templateDetailResult.data.length > 0 && templateDetailResult.data[0].value) {
        const templateData = jsonParse(templateDetailResult.data[0].value)
        return {
          ...templateData,
          versionCode: templateDetailResult.data[0].versionCode,
        }
      }
      return null
    } catch (error) {
